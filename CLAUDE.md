...
- New subcommand, changed flag, new env var, new config field, new install
  step, changed default → validate `README.md` and update it too.

Out-of-date docs are worse than missing docs. If unsure whether a change
is significant enough, update them anyway.

## README and docs — keep them in sync

`README.md` and `docs/*.md` are the canonical references. They must stay
accurate after every change. **Do not finish a task without checking and
updating them.**

**Rule: read the relevant file(s) before closing any task that touches
what they describe. Update them in the same commit if anything changed.**

Triggers for `README.md`:

- CLI subcommands (`parley <cmd>`) — add, remove, rename, or flag changes.
...
Triggers for `docs/architecture.md`:

- New endpoint or changed HTTP contract.
- New or changed data shape, wire format, or protocol invariant.
- New env var that affects server or client behaviour.
- Changes to audience expansion, hub locking, or fan-out behaviour.

Triggers for `docs/tasks.md`:

- Any task completed, descoped, or added.

If unsure whether a change is significant enough to warrant a doc update,
update anyway. Out-of-date docs are worse than missing docs.
...
to avoid a macOS codesign-cache bug where overwriting an existing binary in
place leaves a stale cached signature that causes SIGKILL on the next exec.
Never change the install step to plain `cp`.
